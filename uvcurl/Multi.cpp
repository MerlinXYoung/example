...
static curl_context_t *_create_curl_context(Multi* multi, curl_socket_t sockfd) {
    curl_context_t *context = uvcurl_malloc(curl_context_t);//reinterpret_cast<curl_context_t*>(malloc(sizeof(curl_context_t)));
...
Multi::Multi():loop_(nullptr),curlm_(nullptr,curl_multi_cleanup)
{
...
int Multi::init(uv_loop_t& loop)
{
    loop_ = &loop;
    int ires = uv_timer_init(loop_, &timer_);
    if(ires != 0)
        return -1;
    timer_.data = this;
    curlm_.reset(curl_multi_init());
